    ...
    for (const source in solcOutput.contracts) {
        ...
        for (const contractDef of utils_1.findAll('ContractDefinition', solcOutput.sources[source].ast)) {
            ...
            if (contractDef.name in validation && bytecode !== undefined) {
                ...
                validation[contractDef.name].methods = [...utils_1.findAll('FunctionDefinition', contractDef)]
                    .filter(fnDef => ['external', 'public'].includes(fnDef.visibility))
                    .map(fnDef => function_1.getFunctionSignature(fnDef, deref));
            }
        }
    }
...
function* getConstructorErrors(contractDef, decodeSrc) {
    var _a, _b;
    for (const fnDef of utils_1.findAll('FunctionDefinition', contractDef, node => skipCheck('constructor', node))) {
...
function* getOpcodeErrors(contractDef, decodeSrc) {
    var _a, _b;
    for (const fnCall of utils_1.findAll('FunctionCall', contractDef, node => skipCheck('delegatecall', node))) {
    ...
    for (const fnCall of utils_1.findAll('FunctionCall', contractDef, node => skipCheck('selfdestruct', node))) {
        const fn = fnCall.expression;
